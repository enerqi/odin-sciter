...
| `Element.prototype.morphContent` / `replaceContent` | `morphContent(step, {duration, ease})` calls `step(progress: 0…1)` at frame rate with ~25 named easings; `replaceContent(jsx, {effect})` swaps content with a named slide/blend effect and awaits | **Script only, and the right tool when a tween is needed** — there is no host animation API, and a host `setInterval`-style loop across the boundary is the wrong shape. Most moves need neither: a CSS `transition` on `transform` animates on the engine's frame clock, provided the transform is spelled the one way the engine accepts (`html-css-js.md`) |
